...
import { auditService, type AuditLogEntry } from "../../security/audit.service";
import { monitoringService, type SystemHealthStatus } from "../../security/monitoring";
  ...
  return (
    <div style={{ position: "relative" }} className="admin-signals-wrapper">
      ...
      <div className="admin-header-row" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "2rem 2rem 1rem", borderBottom: "1px solid var(--border)", flexWrap: "wrap", gap: "1rem" }}>
        ...
        {/* Tab Switcher */}
        <div style={{ display: "flex", background: "rgba(0,0,0,0.25)", border: "1px solid var(--border)", borderRadius: "8px", padding: "4px", gap: "4px" }}>
          <button
            ...
            style={{ 
              ...
              border: "none", 
              borderRadius: "6px", 
              background: activeTab === "console" ? "var(--accent-purple)" : "transparent", 
              color: "#fff", 
              fontSize: "13px", 
              fontWeight: "600", 
              cursor: "pointer", 
          ...
          <>
            ...
            {activeTab === "security" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                ...
                {/* Diagnostic Health Cards / Status Bars */}
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1.5rem" }}>
                  {/* API Gateway status */}
                  <Card 
                    title="API Gateway Status" 
                    subtitle="Vite Dev Server / Proxy layer" 
                    extra={<Server size={18} color={systemHealth?.apiStatus === "Healthy" ? "var(--accent-green)" : "var(--accent-red)"} />}
                  >
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Status</span>
                        <Badge variant={systemHealth?.apiStatus === "Healthy" ? "success" : "danger"} style={{ fontWeight: "700" }}>
                          {systemHealth?.apiStatus || "Unknown"}
                        </Badge>
                      </div>
                      
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Latency</span>
                        <strong style={{ fontSize: "14px", color: "#fff" }}>{systemHealth?.apiLatencyMs || 0} ms</strong>
                      </div>
                      ...
                      <div style={{ width: "100%", height: "6px", background: "rgba(255,255,255,0.05)", borderRadius: "3px", overflow: "hidden", marginTop: "4px" }}>
                        <div 
                          style={{ 
                            height: "100%", 
                            width: `${Math.min(100, ((systemHealth?.apiLatencyMs || 35) / 150) * 100)}%`, 
                  ...
                  <Card 
                    ...
                    subtitle="Postgres Schema & RLS validation" 
                    extra={<Database size={18} color="var(--accent-green)" />}
                  >
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Connection</span>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Ping Response</span>
                        <strong style={{ fontSize: "14px", color: "#fff" }}>{systemHealth?.dbPingMs || 0} ms</strong>
                      </div>
                      ...
                      <div style={{ width: "100%", height: "6px", background: "rgba(255,255,255,0.05)", borderRadius: "3px", overflow: "hidden", marginTop: "4px" }}>
                        <div 
                          style={{ 
                            height: "100%", 
                            width: `${Math.min(100, ((systemHealth?.dbPingMs || 15) / 50) * 100)}%`, 
                            background: "linear-gradient(90deg, var(--accent-green), #10B981)", 
                            borderRadius: "3px" 
                          }} 
                        />
                  ...
                  {/* AI Engine status */}
                  <Card 
                    title="AI Score & Timing Engine" 
                    subtitle="TIC Scoring Engine status" 
                    extra={<Terminal size={18} color="var(--accent-green)" />}
                  >
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Engine State</span>
                        <Badge variant="success" style={{ fontWeight: "700" }}>
                          {systemHealth?.aiStatus || "Online"}
                        </Badge>
                      </div>
                      
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: "14px", color: "var(--text-secondary)" }}>Inference Latency</span>
                        <strong style={{ fontSize: "14px", color: "#fff" }}>{systemHealth?.aiLatencyMs || 0} ms</strong>
                      </div>
                      ...
                      <div style={{ width: "100%", height: "6px", background: "rgba(255,255,255,0.05)", borderRadius: "3px", overflow: "hidden", marginTop: "4px" }}>
                        <div 
                          style={{ 
                            height: "100%", 
                            width: `${Math.min(100, ((systemHealth?.aiLatencyMs || 95) / 300) * 100)}%`, 
                            background: "linear-gradient(90deg, var(--accent-green), #10B981)", 
                            borderRadius: "3px" 
                          }} 
                        />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1.8fr", gap: "2rem" }} className="responsive-split-row">
                  {/* Left Column: Diagnostics Controls & Captured Exception logs */}
                  <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                    <Card title="Diagnostics Console" subtitle="Manual override & active monitor controls">
                      <div style={{ display: "flex", gap: "12px", marginBottom: "15px" }}>
                        ...
                        <Button 
                          variant="secondary" 
                          icon={<Trash2 size={14} color="var(--accent-red)" />} 
                          onClick={handleClearSentryLogs}
                          style={{ flex: 1, padding: "8px 12px", fontSize: "12px" }}
                        >
                          Clear Errors
                        </Button>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px", background: "rgba(16, 185, 129, 0.05)", border: "1px solid rgba(16, 185, 129, 0.15)", borderRadius: "10px" }}>
                        <ShieldCheck size={20} color="var(--accent-green)" />
                        <div style={{ fontSize: "12.5px" }}>
                          <span style={{ color: "#fff", display: "block", fontWeight: "600" }}>Secure Shell Active</span>
                          <span style={{ color: "var(--text-secondary)", fontSize: "11px" }}>Active Developer Sessions: {systemHealth?.activeSessions || 1}</span>
                        </div>
                      </div>
                    </Card>

                    <Card title="Captured Exceptions (Mock Sentry)" subtitle="Captured errors in memory logs">
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px", maxHeight: "300px", overflowY: "auto", paddingRight: "4px" }}>
                        {systemHealth?.errors && systemHealth.errors.length > 0 ? (
                          systemHealth.errors.map((err, idx) => (
                            <div 
                              key={idx} 
                              style={{ 
                            ...
                            >
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "4px" }}>
                                <Badge variant="danger" style={{ fontSize: "10px", fontWeight: "800" }}>EXCEPTION</Badge>
                                <span style={{ fontSize: "10px", color: "var(--text-secondary)" }}>
                                  {new Date(err.timestamp).toLocaleTimeString()}
                                </span>
                              </div>
                              <div style={{ fontSize: "12px", color: "#fff", fontWeight: "500", wordBreak: "break-all" }}>{err.message}</div>
                              {err.stack && (
                                <pre style={{ margin: "6px 0 0 0", padding: "6px", background: "rgba(0,0,0,0.3)", borderRadius: "4px", fontSize: "9.5px", color: "var(--text-secondary)", overflowX: "auto" }}>
                                  {err.stack.split("\n").slice(0, 3).join("\n")}
                                </pre>
                              )}
                            </div>
                          ...
                          <div style={{ padding: "2rem", textAlign: "center", color: "var(--text-secondary)", fontSize: "13px" }}>
                            All systems nominal. No errors reported to Sentry.
                          </div>
                        )}
                      </div>
                    </Card>
                  ...
                  <div>
                    <Card title="Security Access & Audit Trail" subtitle="Enforced client security validations & DB row actions" extra={<FileSpreadsheet size={16} />}>
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px", maxHeight: "500px", overflowY: "auto", paddingRight: "4px" }}>
                        {securityAuditLogs.length > 0 ? (
                            ...
                            >
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "6px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                                  <strong style={{ color: "#fff", fontSize: "13px" }}>{log.action}</strong>
                                  <Badge variant="info" style={{ fontSize: "9.5px", background: "rgba(255,255,255,0.05)", border: "1px solid var(--border)" }}>IP: {log.ip_address}</Badge>
                                </div>
                              ...
                              <div style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
                                User ID: <span style={{ fontFamily: "monospace", color: "var(--text-primary)" }}>{log.user_id}</span>
                              </div>

                              {log.resource_type && (
                                <div style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
                                  Resource: <span style={{ color: "var(--text-primary)" }}>{log.resource_type} ({log.resource_id})</span>
                                </div>
                              )}

                              {log.metadata && Object.keys(log.metadata).length > 0 && (
                                <div style={{ marginTop: "4px", padding: "8px", background: "rgba(0,0,0,0.25)", border: "1px solid var(--border)", borderRadius: "6px" }}>
                                  <div style={{ fontSize: "10px", color: "var(--text-secondary)", fontWeight: "700", textTransform: "uppercase", marginBottom: "4px" }}>Payload / Metadata</div>
                                  <pre style={{ margin: 0, fontSize: "10px", color: "var(--accent-purple)", overflowX: "auto" }}>
                          ...
                          <div style={{ padding: "2rem", textAlign: "center", color: "var(--text-secondary)", fontSize: "13px" }}>
                            No security actions recorded in the current session.
                          </div>
                        )}
                      </div>
                    </Card>
